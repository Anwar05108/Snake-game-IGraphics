...
void iDraw()
{
    ...
    else if(gamestate==0)
    {
        ...
        {
            if (j==0)
               iShowBMP2(headX,headY,"Head4.bmp", 1);
            else if (j<Count-1)
        ...
        snake_death();
        if (headX<10 || headY<50 || headY>650 || headX>930)
            gamestate=1;
        if (sqrt(pow(headX-fruitX,2)+pow(headY-fruitY,2))<=1)
        {
            FruitSpawn();
            score+=10;
            Count++;
        ...
        else if (gamestate==1)
    {
        ...
        iText(600,300,"YOUR SCORE is %s",GLUT_BITMAP_TIMES_ROMAN_24);
    }
}
...
int main()
{
    ...
    for(int i=2; i>=0; i--)
    {
        ...
        FruitSpawn();
        Count=3;
        iSetTimer(15,position);
        iSetTimer(15,position_update);
