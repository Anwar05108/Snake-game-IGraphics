...
void iDraw()
{
    ...
    else if(gamestate==0)
    {
        ...
        score_str[0]='2';score_str[1]='0';score_str[2]='1';score_str[3]='\0';
        iSetColor(0,0,0);
...
//        iText(600,360,"GAME OVER",GLUT_BITMAP_TIMES_ROMAN_24);
        ...
        iText(550,100,"Press any key to Exit",GLUT_BITMAP_TIMES_ROMAN_24);
        for (i=0; i<3; i++)
        {
            f=0;
            if (strcmp(score_str,score_var[i].score_value)>=0)
            {
                 for (j=i; j<3; j++)
                {
                    if (j==i)
                    {
                        strcpy(temnxt,score_var[j].score_value);
                        strcpy(temstrnxt,score_var[j].name);
                        strcpy(score_var[j].score_value,score_str);
                        strcpy(score_var[j].name,h_name);
                        f=1;
                    }
                    else if (j!=i && f==1)
                    {
                        strcpy(tem,score_var[j].score_value);
                        strcpy(temstr,score_var[j].name);
                        strcpy(score_var[j].score_value,temnxt);
                        strcpy(score_var[j].name, temstrnxt);
                        strcpy(temnxt,tem);
                        strcpy(temstrnxt,temstr);
                    }
                }
            }
            if (f==1)
